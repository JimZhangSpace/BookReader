...
public class OkHttpManager {
    ...
    public static String token = "20180213";
    ...
    public void getRequest(String url, final BaseCallBack callBack) {
        String resultUrl;
        if(url.startsWith("/")) {
            resultUrl = RoadShowBusiness.URL.substring(0,RoadShowBusiness.URL.length()-1)+url;
        }else{
            resultUrl=RoadShowBusiness.URL+url;
        }
        Request request = buildRequest(resultUrl, null, HttpMethodType.GET);
    ...
    public void postJsonRequest(String url, final BaseCallBack callBack, String Jsonparams) {
        String resultUrl=RoadShowBusiness.URL+url;
        Request request = buildJsonRequest(resultUrl, Jsonparams, HttpMethodType.POST);
        doRequest(request, callBack);
    }
...
//                        callBack.onSuccess(call, response, result);
    ...
    private Request buildRequest(String url, Map<String, String> params, HttpMethodType methodType) {
        ...
        String urlp = String.format("%s?clientType=android&uid=%s&token=%s&innerVersion=%s&role=agent",url,uid,token,AppConstant.inner_version);
        ...
        builder.url(urlp);
//        RequestBody.create(MEDIA_TYPE_MARKDOWN, params)
    ...
    //创建 Request对象
    private Request buildJsonRequest(String url, String Jsonparams, HttpMethodType methodType) {
        Request.Builder builder = new Request.Builder();
        //android
        String urlp = String.format("%s?clientType=android&uid=%s&token=%s&innerVersion=%s&role=agent",url,uid,token,AppConstant.inner_version);
        String HttpToken = SharePrefUtil.getString("HttpToken",null);
        ...
        if (methodType == HttpMethodType.GET) {
            ...
            builder.post(RequestBody.create(MEDIA_TYPE_MARKDOWN, Jsonparams));
        }
        return builder.build();
    }
