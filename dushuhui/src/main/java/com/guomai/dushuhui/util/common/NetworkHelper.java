...
public class NetworkHelper {
	...
	{
//		PLog.v(TAG, "registerNetworkSensor");
		if (mRegistered)
			return;
		...
		{
//			PLog.i(TAG, "network not reachable");
			mStatus = NetworkStatus.NetworkNotReachable;
		}
		else if (info.getType() == ConnectivityManager.TYPE_MOBILE)
		{
//			PLog.i(TAG, "network reachable via wwan");
			mStatus = NetworkStatus.NetworkReachableViaWWAN;			
			
		}
		else if (info.getType() == ConnectivityManager.TYPE_WIFI)
		{
//			PLog.i(TAG, "network reachable via wifi");
...
//			PLog.v("NetworkBroadcastReceiver", "onReceive");
			if (intent == null)
				return;
			...
			if (action.equals(ConnectivityManager.CONNECTIVITY_ACTION))
			{
				...
				if (info == null || !info.isAvailable())
				{
//					PLog.i("NetworkBroadcastReceiver", "network not reachable");
					ns = NetworkStatus.NetworkNotReachable;
				}
				else if (info.getType() == ConnectivityManager.TYPE_MOBILE)
				{
//					PLog.i("NetworkBroadcastReceiver", "network reachable via wwan");
					ns = NetworkStatus.NetworkReachableViaWWAN;			
					
				}
				else if (info.getType() == ConnectivityManager.TYPE_WIFI)
				{
//					PLog.i("NetworkBroadcastReceiver", "network reachable via wifi");
